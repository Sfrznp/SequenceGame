...
def check_sequence(row, col, dr, dc, board, player):
    # Check for potential sequences in a given direction
    sequence = 0
    for i in range(win_num_tokens):
        r, c = row + i * dr, col + i * dc
        if 0 <= r < NUM_ROWS and 0 <= c < NUM_COLS and board[r][c] == player:
            sequence += 1
        else:
            break
    return sequence
